...
export interface BashOptions {
  ...
  /** Embedding hook used to materialize a network policy without importing it. */
  secureFetchFactory?: (config: NetworkConfig) => SecureFetch;
  /**
   ...
   */
  commands?: string[];
  /** Built-in commands supplied by an embedding-specific entry point. */
  bundledCommands?: Command[];
  /**
  ...
  /**
   * Host clock used for execution deadlines and the initial shell timestamp.
   * Embeddings with durable clocks can provide a pure logical clock here.
   */
  now?: () => number;
  /**
...
export class Bash {
  ...
  constructor(options: BashOptions = {}) {
    this.nowFn = options.now ?? Date.now.bind(Date);
    ...
    if (options.fetch) {
      ...
      if (!options.secureFetchFactory) {
        throw new Error(
          "network requires an embedding-provided secureFetchFactory",
        );
      }
      this.secureFetch = options.secureFetchFactory(options.network);
    }
    ...
    for (const cmd of options.bundledCommands ?? []) {
  ...
  async exec(
    ...
    // @banned-pattern-ignore: EmbeddedBash owns the top-level execution budget.
    ...
    try {
      ...
      try {
        ...
        const executeScript = async (): Promise<BashExecResult> => {
          ...
          // @banned-pattern-ignore: this interpreter reuses the execution scope above.
