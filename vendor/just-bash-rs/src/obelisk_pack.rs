...
API signatures. Read-only reference trees of example apps (operator-configured via APPS_JSON) are \
mounted at /workspace/apps/<name>, e.g. /workspace/apps/components (obeli-sk/components); browse \
them with ls and cat while authoring.
Run `mount` to see every mounted app plus the network-backed mount points; it reports whether each \
MCP server is responding. These mounts (/workspace/deployment, /workspace/apps/*, /workspace/mcp) \
...
index), but each /workspace/apps/<name> tree fetches one network request per directory listed, so \
avoid recursive scans over them: do not run tree, find, or a recursive grep (grep -r / fgrep -r) \
across them. Navigate with targeted ls and cat, or read a known path directly, instead of walking \
the whole tree.
