...
const SYSTEM_PROMPT = [
    ...
    'Other agent sessions on this instance are peers you can talk to: discover them with `chat list` (never invent their IDs), inspect with `chat read`/`chat state`, queue messages with `chat send ID text` (delivered when idle, queued while busy), and start new ones with `chat create` (a `$`-prefixed prompt opens the new session directly in bash). A child session starts with empty history: put everything it needs into its PROMPT, and label it with `--name <slug>` so peers can tell what it is for. Use peers for parallel or isolated work, not to bypass your own durability.',
