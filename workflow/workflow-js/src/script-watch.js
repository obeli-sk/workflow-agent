...
// onto a fresh join set named after `scriptId`. PORT: script_watch.rs's
// `ScriptWatchGuard::arm` (Rust's `workflow_support::join_set_create()`
// makes a fresh *unnamed* join set per script; this runtime's
// `createJoinSet` requires a name, and a fixed one is rejected the second
// time a session runs a script - "Failed to create named join set:
// JoinSetCreateError::Conflict" - so each call needs a distinct name).
// `scriptId` is the caller's own shell/tool-call id (e.g. "shell-e2e-1", an
// LLM tool_use id), already unique per script execution by construction, so
// reusing it here needs no separate counter or other synthesized state.
export function arm(timeoutMs, scriptId) {
    const joinSet = obelisk.createJoinSet({ name: `script-watch-${scriptId}` });
